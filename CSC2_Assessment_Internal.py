...
from tkinter import ttk  # This imports ttk from tkinter
from tkinter import messagebox  # This imports messagebox module from tkinter
...
# This function quits the window for 'view customer details'
def close_viewer():
    global new_window
    new_window.destroy()

# Delete function for receipt (view customer details)
def delete_receipt(receipt_click):
    ...
    if messagebox.askyesno('Warning!', 'Are you sure you want to delete this receipt?\nDoing so will delete any '
                                       'associated information/details of this customer including this receipt.',
                           icon='warning'):
        ...
        # instead of an error
        try:
            save_details.pop(receipt_selected)
        except:
            pass
        try:
            customer_details.pop(receipt_selected)
        except:
            pass
        try:
            Check_Receipts.pop(receipt_selected)
            Check_Receipts.append(list(customer_details[receipt_selected][3]))
            # Removes all the tags in the integer list after appending the values to it
...
def print_view_customer_details():
    ...
    global customer_details, customer_name, item_hired, number_of_item_hired, customer_receipt, total_entries, \
        Check_Receipts, total_number_of_entries
    ...
    total_entries = 0
    for how_many_in_list in range(len(total_number_of_entries)):
        total_entries += 1

    add_name = []
    for adding_customer_name in range(len(customer_details)):
        add_name.append(customer_details[adding_customer_name][0])
                ...
                for details in range(len(customer_view_details)):
                    specified_buttons.append(
                        Button(frame2, text="View Details",
                               command=lambda counter=name_count2: view_details(total_number_of_entries[counter][0]))
                        .grid(column=2, row=3 + name_count2))

            if total_number_of_entries[increment_value][0] in add_name:
                Label(frame2, text=f"{total_number_of_entries[name_count2][0]}", font=('bold', 12)).grid(column=1,
                                                                                              row=3 + name_count2)
                customer_view_details.append("Button" + str(name_count2 + 1))

                for details2 in range(len(customer_view_details)):
                    specified_buttons.append(
                        Button(frame2, text="View Details",
                               command=lambda counter=name_count2: view_details(total_number_of_entries[counter][0]))
                        .grid(column=2, row=3 + name_count2))
...
def print_customer_details():
    ...
    while name_count < total_entries:
        ...
        try:  # If an item is in the current name_count value
            Label(frame, text=(customer_details[name_count][0])).grid(column=1, row=name_count + 9, columnspan=1)
        except:  # If an item is in one index behind the current name_count value
            Label(frame, text=(customer_details[name_count - 1][0])).grid(column=1, row=name_count + 9, columnspan=1)
        # Using triple nested list, the item hired from the certain customer is gridded on to the multidimensional list
        # The index of the third list inside customer details is the index number from item hired in customer details -
        # minus by one
        try:
...
def checking_inputs():
    ...
    input_check = 0
    only_letters = True
    check_receipt_value = True
    numbers = []
    other_expression = ["~", "!", "#", "$", "%", "^", "&", "*", "(", ")", "+", "=", ",",
                        "<", ">", "/", "[", "]", "{", "}", "|", ":", ";", '"', "'", "?"]
    # Adding number into a list, an increment of 1
    start_num_value = 0  # Starts at 0
    for adding_number in range(10):  # loop 10 times, so; 0 to 9
    ...
    Label(tab1, text="                    ").grid(column=1, row=6)
    ...
    if len(customer_name.get()) == 0:
        ...
        for checking2 in range(len(check_customer_name)):
            if check_customer_name[checking2] in other_expression:
                only_letters = False
    ...
    if not customer_receipt.get():
        ...
        # If the receipt number is new and not within current one
        if Receipt not in Check_Receipts:
            ...
            # If the receipt number generated is the same as the previous one, it will generate a new one again
    ...
    # randomly generated
    check_receipt = list(customer_receipt.get())
    for checking_receipt in range(len(check_receipt)):
        if check_receipt[checking_receipt] == '#':
            pass
        elif check_receipt[checking_receipt] not in numbers:
            check_receipt_value = False

    if check_receipt_value is False:
        Label(tab1, text="Integer Only", fg='red').grid(column=1, row=6)
        input_check = 1
...
def append_details():
    ...
    total_number_of_entries.append([customer_name.get(), [item_hired.get()], [number_of_item_hired.get()], customer_receipt.get()])
            ...
            if same_name >= 2:
                ...
                for check_customer_list2 in range(len(customer_details)):
                    if customer_details[check_customer_list2][0] == customer_name.get():
                        ...
                        total_number_of_entries[check_customer_list2][2].append(number_of_item_hired.get())
                        total_number_of_entries.pop()
    ...
    if same_name < 2:
        ...
        # This list will have all the names of the current customer
        add_name = []
        for adding_customer_name in range(len(customer_details)):
            add_name.append(customer_details[adding_customer_name][0])
        ...
        increment_value = 0
        while name_count2 < total_entries:
            # In the total number of entries each item is checked if it is within 'add_name' list
            try:
                if total_number_of_entries[increment_value][0] in add_name:
                    Label(frame2, text=f"{total_number_of_entries[name_count2][0]}", font=('bold', 12)).grid(column=1, row=3 + name_count2)
                    customer_view_details.append("Button" + str(name_count2 + 1))
                ...
                # This is backup just in case if the condition checker fails
                Label(frame2, text=f"{total_number_of_entries[increment_value][0]}", font=('bold', 12)).grid(column=1, row=3 + name_count2)
                customer_view_details.append("Button" + str(name_count2 + 1))
                ...
                for details in range(len(customer_view_details)):
                    specified_buttons.append(
                        Button(frame2, text="View Details",
                               command=lambda counter=name_count2: view_details(total_number_of_entries[counter][0]))
                        .grid(column=2, row=3 + name_count2))
            increment_value += 1  # Increases value by 1 everytime it runs one loop
            name_count2 += 1
        if len(customer_view_details) != 0:
            customer_view_details.pop(0)
...
def main():
    ...
    global customer_details, customer_name, item_hired, number_of_item_hired, \
        customer_receipt, total_entries, Check_Receipts, customer_view_details, specified_buttons, save_details, \
        item_list, item_number_list, total_number_of_entries
    ...
    # Creates a new tab from ttk called .Notebook
